...
namespace Nop.Services.Catalog
{
    ...
    public partial class CategoryService : ICategoryService
    {
        ...
        /// <returns>
        /// A task that represents the asynchronous operation
        /// The task result contains the vendor category mapping collection
        /// </returns>
        protected virtual async Task<IList<VendorCategory>> GetVendorCategoriesByVendorIdAsync(int vendorId, int storeId,
            bool showHidden = false)
        {
            if (vendorId == 0)
                return new List<VendorCategory>();

            var customer = await _workContext.GetCurrentCustomerAsync();
            var customerRoleIds = await _customerService.GetCustomerRoleIdsAsync(customer);

            return await _vendorCategoryRepository.GetAllAsync(async query =>
            {
                if (!showHidden)
                {
                    var categoriesQuery = _categoryRepository.Table.Where(c => c.Published);
            ...
            }, cache => _staticCacheManager.PrepareKeyForDefaultCache(NopCatalogDefaults.VendorCategoriesByVendorCacheKey,
                vendorId, showHidden, customerRoleIds, storeId));
        }
        ...
        /// The task result contains the vendor category mapping collection
        /// </returns>
        public virtual async Task<IList<VendorCategory>> GetVendorCategoriesByVendorIdAsync(int vendorId, bool showHidden = false)
        {
            var store = await _storeContext.GetCurrentStoreAsync();

            return await GetVendorCategoriesByVendorIdAsync(vendorId, store.Id, showHidden);
        }

        /// <summary>
        /// Deletes a vendor category mapping
        /// </summary>
        /// <param name="vendorCategory">Vendor category</param>
        /// <returns>A task that represents the asynchronous operation</returns>
        public virtual async Task DeleteVendorCategoryAsync(VendorCategory vendorCategory)
        {
            await _vendorCategoryRepository.DeleteAsync(vendorCategory);
        }

        /// <summary>
        /// Returns a VendorCategory that has the specified values
        /// </summary>
        /// <param name="source">Source</param>
        /// <param name="vendorId">Vendor identifier</param>
        /// <param name="categoryId">Category identifier</param>
        /// <returns>A ProductCategory that has the specified values; otherwise null</returns>
        public virtual VendorCategory FindVendorCategory(IList<VendorCategory> source, int vendorId, int categoryId)
        {
            foreach (var vendorCategory in source)
                if (vendorCategory.VendorId == vendorId && vendorCategory.CategoryId == categoryId)
        ...
        /// </summary>
        /// <param name="vendorCategory">>Vendor category mapping</param>
        /// <returns>A task that represents the asynchronous operation</returns>
        public virtual async Task InsertVendorCategoryAsync(VendorCategory vendorCategory)
        {
